...
class FraudAnalysisRepositoryIntegrationTest {
    ...
    void shouldSaveAndRetrieveAnalysisWithMultipleReasons() {
        ...
        assertThat(saved.transactionId())
                .isEqualTo(
                        transaction.id()
                );

        assertThat(saved.riskScore())
                .isEqualTo(
                        analysis.riskScore()
                );

        assertThat(saved.riskLevel())
                .isEqualTo(
                        RiskLevel.HIGH
                );
        ...
        assertThat(saved.reasons())
                .containsExactly(
                        FraudReason.UNUSUAL_AMOUNT,
                        FraudReason.UNKNOWN_DEVICE
                );

        assertThat(saved.analyzedAt())
                .isEqualTo(
                        analysis.analyzedAt()
                );
    ...
    void shouldSaveAnalysisWithoutFraudReasons() {
        ...
        assertThat(retrieved.reasons())
                .isEmpty();

        assertThat(retrieved.decision())
                .isEqualTo(
                        FraudDecision.APPROVED
                );

        assertThat(retrieved.riskLevel())
                .isEqualTo(
                        RiskLevel.LOW
                );

        assertThat(retrieved.riskScore())
                .isEqualTo(
                        RiskScore.zero()
                );
    }
    ...
    void shouldFindAnalysisByTransactionId() {
        ...
        assertThat(retrieved.transactionId())
                .isEqualTo(
                        transaction.id()
                );
