...
class TransactionRepositoryIntegrationTest {
    ...
    void shouldSaveAndRetrieveTransaction() {
        ...
        assertThat(saved.id()).isEqualTo(transaction.id());
        assertThat(saved.userId()).isEqualTo(transaction.userId());
        assertThat(saved.amount()).isEqualTo(transaction.amount());
        assertThat(saved.merchant()).isEqualTo(transaction.merchant());
        assertThat(saved.category()).isEqualTo(transaction.category());
        assertThat(saved.timestamp()).isEqualTo(transaction.timestamp());
        assertThat(saved.location()).isEqualTo(transaction.location());
        assertThat(saved.deviceId()).isEqualTo(transaction.deviceId());

        Transaction retrieved =
                transactionRepository.findById(transaction.id().value()).orElseThrow();
